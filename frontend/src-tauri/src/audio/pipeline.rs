...
const WHISPER_ROLLOVER_OVERLAP_SAMPLES: usize = 48_000; // 3.0s at 16kHz
...
pub struct AudioCapture {
    ...
    // AGC processor for boosting quiet speech before noise reduction
    agc: Arc<std::sync::Mutex<Option<AgcProcessor>>>,
    // GTCRN deep learning denoiser (primary), RNNoise is fallback
    gtcrn_denoiser: Arc<Option<GtcrnDenoiser>>,
...
impl AudioCapture {
    pub fn new(
        ...
        let (noise_suppressor, high_pass_filter, normalizer, agc, gtcrn_denoiser) = if matches!(device_type, DeviceType::Microphone) {
            ...
            let agc = if super::ffmpeg_mixer::is_agc_enabled() {
                ...
                info!("✅ AGC processor initialized for microphone '{}' (target: {:.3} RMS, max gain: {:.1}x)",
            ...
            // Check if all audio enhancement should be skipped
            // (env var VOXMINUTES_SKIP_AUDIO_ENHANCEMENT=1 bypasses all denoising for debugging)
            let skip_audio_enhancement = std::env::var("VOXMINUTES_SKIP_AUDIO_ENHANCEMENT")
                .map(|v| v == "1" || v.eq_ignore_ascii_case("true"))
                .unwrap_or(false);

            if skip_audio_enhancement {
                warn!("⚠️ VOXMINUTES_SKIP_AUDIO_ENHANCEMENT is set — skipping all denoising");
            }

            // GTCRN deep learning denoiser has been permanently disabled.
            // GTCRN was causing 30-40% CPU usage even with idle audio, due to
            // ONNX Runtime CPU inference running on every 30ms audio chunk.
            // RNNoise (lightweight C implementation, 10-15 dB reduction) is used instead.
            info!("ℹ️ GTCRN disabled — using RNNoise lightweight denoiser (10-15 dB) for microphone '{}'", device.name);
            let gtcrn_denoiser = GtcrnDenoiser::empty();

            // Initialize noise suppression (RNNoise) at 48kHz
            let ns = if skip_audio_enhancement {
                warn!("   Skipping RNNoise: disabled by VOXMINUTES_SKIP_AUDIO_ENHANCEMENT");
                None
            } else if super::ffmpeg_mixer::is_rnnoise_enabled() {
                ...
                info!("ℹ️ RNNoise noise suppression DISABLED for microphone '{}' (flag: is_rnnoise_enabled=false)", device.name);
            ...
            let norm = if super::ffmpeg_mixer::is_ebu_enabled() {
                ...
                info!("ℹ️ EBU R128 normalization DISABLED for microphone '{}' (flag: is_ebu_enabled=false)", device.name);
                None
            };
        ...
        Self {
            ...
            agc: Arc::new(std::sync::Mutex::new(agc)),
            gtcrn_denoiser: Arc::new(gtcrn_denoiser),
    ...
    pub fn process_audio_data(&self, data: &[f32]) {
        ...
        // Microphone mute: zero out audio BEFORE level reporting and pipeline.
        // This ensures the UI energy bar shows silence and no audio enters transcription.
        if matches!(self.device_type, DeviceType::Microphone) && self.state.is_mic_muted() {
            mono_data.iter_mut().for_each(|s| *s = 0.0);
        }
        ...
        if !mono_data.is_empty() {
            ...
            super::recording_state::update_spectrum_ring(self.sample_rate, &mono_data);
        }
        ...
        // Processing order is critical: high-pass → AGC → denoise → normalization
        // AGC boosts quiet speech before denoising, giving the denoiser a stronger
        // signal-to-noise ratio to work with. Noise is removed before normalization.
        if matches!(self.device_type, DeviceType::Microphone) {
            // If microphone is muted, replace all samples with silence.
            // This ensures: ring-buffer mixing still works, VAD won't falsely
            // trigger on ambient noise, recording file gets clean silence for mic,
            // and X-ASR streaming receives zero data instead of garbage audio.
            if self.state.is_mic_muted() {
                mono_data.iter_mut().for_each(|s| *s = 0.0);
            } else {
                ...
                // STEP 2: Apply AGC to boost quiet speech before denoising
                // This improves SNR for the denoiser, leading to better noise suppression
                if let Ok(mut agc_lock) = self.agc.lock() {
                    if let Some(ref mut agc) = *agc_lock {
                        let before_len = mono_data.len();
                        mono_data = agc.process(&mono_data);
                        let after_len = mono_data.len();
                ...
                // STEP 3: Apply speech enhancement (GTCRN primary, RNNoise fallback)
                // GTCRN: >20 dB noise reduction with deep learning
                // RNNoise: 10-15 dB noise reduction with lightweight neural network
                if let Some(ref gtcrn) = *self.gtcrn_denoiser {
                    if gtcrn.is_available() {
                        mono_data = gtcrn.process(&mono_data, 48000);
                    }
                }
                        ...
                        if let Ok(mut ns_lock) = self.noise_suppressor.lock() {
                            if let Some(ref mut suppressor) = *ns_lock {
                                ...
                                if chunk_id % 100 == 0 {
                                    ...
                                    debug!("🔇 RNNoise health: in={}, out={}, delta={}, buffered={}, RMS={:.4}",
    ...
    pub fn handle_stream_error(&self, error: cpal::StreamError) {
        ...
        // Notify the default-device follower that this stream needs rebuilding.
        self.state.set_stream_failed(self.device_type.clone(), true);
...
impl AudioPipeline {
    ...
    fn maybe_emit_whisper_live_preview(&mut self) {
        ...
        // sentence context, but short enough for realtime translation. Keep 3s
        // overlap so words cut at the hard boundary can be recovered next window.
    ...
    pub async fn run(mut self) -> Result<()> {
        if self.bypass_vad {
            info!("X-ASR mode: VAD bypassed - sending continuous mixed audio directly");
        } else {
                ...
                Ok(Some(chunk)) => {
                    ...
                    while self.ring_buffer.can_mix() {
                        if let Some((mic_window, sys_window)) = self.ring_buffer.extract_window() {
                            ...
                            // POST-GAIN: Only applied when EBU R128 normalization is disabled
                            // When EBU R128 is off, raw microphone audio may need a small boost
                            let mixed_with_gain = mixed_clean;

                            // ─── DIAGNOSTIC: Log mixed audio RMS every ~10s ───
