...
impl RecordingManager {
    ...
    /// Rebuild the audio streams using the current system default devices.
    /// Keeps the pipeline, saver, and recording session alive.
    pub async fn restart_streams_with_current_defaults(&mut self) -> Result<()> {
        info!("🔄 Rebuilding audio streams with current default devices");

        // Resolve the devices we should use now.
        let new_mic = if self.follow_mic {
            default_input_device().ok().map(Arc::new)
        } else {
            self.state.get_microphone_device()
        };

        let new_sys = if self.follow_system {
            default_output_device().ok().map(Arc::new)
        } else {
            self.state.get_system_device()
        };

        // Microphone is required. If we are following the default mic and none is
        // available, stop streams and wait instead of failing the whole session.
        if new_mic.is_none() && self.follow_mic {
        ...
        // Stop existing streams before recreating them. A short sleep helps CPAL
        // release the previous device on Windows.
        if let Err(e) = self.stream_manager.stop_streams() {
            warn!("Error stopping streams before rebuild: {}", e);
        }
        tokio::time::sleep(tokio::time::Duration::from_millis(100)).await;

        // Start new streams. System audio is optional: if it is missing we keep
        // recording with the microphone only.
        self.stream_manager
            .start_streams(new_mic.clone(), new_sys.clone(), None)
            .await?;

        // Update state to reflect the devices now in use.
        if let Some(ref d) = new_mic {
            self.state.set_microphone_device(d.clone());
        }
        if let Some(ref d) = new_sys {
            self.state.set_system_device(d.clone());
        }
        self.state.set_current_default_names(
    ...
    /// Request the default-device follower to check defaults on the next iteration.
    pub fn check_default_devices_now(&self) {
        self.state.set_pending_device_check(true);
    }

    /// Check if the session is currently waiting for a microphone to return.
    pub fn is_waiting_for_device(&self) -> bool {
        self.state.is_waiting_for_device()
    }
